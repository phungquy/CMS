...
class News extends MY_Controller {
	...
	public function add()
	{
	        ...
	        if (isset($_POST['news_title'])) {
				...
				$news_secondary_category = $this->input->post('news_secondary_category');
				$news_secondary_category = !empty($news_secondary_category) ? ',' . implode(',', $news_secondary_category) . ',' : '';
	        	$this->_data['formData'] = array( 
		        	'news_category' => $this->input->post('news_category'), 
					'news_secondary_category' => $news_secondary_category,
		...
		$this->_data['secondary_category'] = $this->mcategory->dropdownlistCategory($this->_data['formData']['news_secondary_category'],$this->_data['langPost']['lang_code'],'news');
				...
				if ($myNews && $myNews['id'] > 0) {
					if ($myNews['news_state'] == 3 && $this->mpermission->permission("news_editpublish",$this->_data['user_active']['active_user_group']) != true) {
		                ...
		                $this->_data['formData'] = array( 
				        	...
				        	'news_secondary_category' => !empty($myNews['news_secondary_category']) ? explode(',', trim($myNews['news_secondary_category'], ',')) : '',
			        ...
			        if (isset($_POST['news_title'])) {
			            ...
			            if ($error == true) {
				            ...
				            switch ($method) {
		                    		...
		                    		break;
			                    	case 2:
			                    		redirect(my_library::admin_site()."news/index/".$myNews['news_state']."?fcategory=".$this->_data['formData']['news_category']);
		                    		break;
			                    	case 3:
			                    		redirect(my_library::admin_site()."news/index/".$state."?fcategory=".$this->_data['formData']['news_category']);
		                    		break;
			                    	default:
			                    		redirect(my_library::admin_site()."news/edit/".$id);
		                    		break;
