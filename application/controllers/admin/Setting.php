...
class Setting extends MY_Controller {
    ...
    public function index()
    {
        ...
        $this->_data['formData'] = array(
            ...
            'use_cache' => $mySetting['use_cache'],
            'time_cache' => $mySetting['time_cache']
        );
        if (isset($_POST['limit_title'])) {
            ...
            $this->_data['formData'] = array(
	            ...
	            'use_cache' => $this->input->post('use_cache') ?? 0,
	            'time_cache' => $this->input->post('time_cache')
            );
            $error = false;
            do {
                ...
                if ($this->_data['formData']['use_cache'] == 1) {
                	if ($this->_data['formData']['time_cache'] < 1) {
                        $text = lang('pleaseinput').lang('timecache').' cache!';$error = true;break;
                    }
                }
            } while (0);
            if ($error == true) {
