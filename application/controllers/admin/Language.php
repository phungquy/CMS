        ...
        if (isset($_POST['lang_name'])) {
            ...
            if ($error == true) {
                ...
                if (is_numeric($insert) > 0) {
                    $dirLangDefault = APPPATH . '/language/english';
                    $dirLangDefaultFront = APPPATH . '/language/english_front';
                    $dir = APPPATH . '/language/'.$this->_data['formData']['lang_code'];
                    $dirFront = APPPATH . '/language/'.$this->_data['formData']['lang_code'].'_front';
                    if(!is_dir($dir)){
                        mkdir($dir, 0755, true);
                        chmod($dir, 0755);
                    }
                    if(!is_dir($dirFront)){
                        mkdir($dirFront, 0755, true);
                        chmod($dirFront, 0755);
                    }
                    xcopy($dirLangDefault,$dir);
                    xcopy($dirLangDefaultFront,$dirFront);
                    $notify = array(
                        'title' => lang('success'), 
