...
class mconfig extends MY_Model
{
    ...
    public function getConfig($lang='vietnamese',$limit = '')
    {
        ...
        // }
        $sql = 'select c.id,c.config_code,c.config_status,c.config_all,c.config_user,ct.id as id_lang,ct.config_name,ct.config_value from '.$this->table.' c inner join '.$this->table_translation.' ct on c.id = ct.config_id where ct.language_code = "'.$lang.'" order by c.id asc';
        if ($limit != '') {
            $sql .= ' limit '.$limit;
        }
