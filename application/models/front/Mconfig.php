...
class mconfig extends CI_Model
{
    ...
    public function getConfig($lang='vietnamese',$all=0)
    {
        $sql = 'select c.config_code,ct.config_name,ct.config_value from '.$this->table.' c inner join '.$this->table_translation.' ct on c.id = ct.config_id where c.config_status = 1 and c.config_all = 1 and ct.language_code = "'.$lang.'"';
        if ($all == 1) {
        ...
        foreach ($list as $value) {
            ...
            $rs[$value['config_code']] = $value['config_value'];
        }
        return $rs;
    }
}
