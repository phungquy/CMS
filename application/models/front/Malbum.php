...
class malbum extends CI_Model
{
    ...
    public function getAlbumDetail($album_id,$obj='*',$limit='')
    {
    ...
    {
        if ($category_id) {
            $sql = 'select '.$obj.' from '.$this->table.' a inner join '.$this->table_translation.' at on a.id = at.album_id where a.album_parent = '.$category_id.' and a.album_status = 1 and at.language_code = "'.$lang.'" order by a.album_orderby asc,a.id desc';
            if ($limit) {
                $sql .= ' limit '.$limit;
            }
            $query = $this->db->query($sql);
