...
class Shortcodes
{
    ...
    public function parse ($str)
    {
        ...
        if (! strstr($str, '[intynets:')) {
            return $str;
        }
        // Find matches against shortcodes like [ai:user id=1|class=admin]
        
        //preg_match_all('/\[ai:([a-zA-Z0-9-_: |=\.]+)]/', $str, $shortcodes);
        preg_match_all('/\[intynets:([a-zA-Z0-9-_:( )(&nbsp;)|=\.]+)]/', $str, $shortcodes);
        $shortcodes[1] = str_replace('&nbsp;', ' ', $shortcodes[1]);
        if ($shortcodes == NULL) {
            return $str;
        }
