							...
							<tbody>
								<?php if (!empty($list)): ?>
									<?php foreach ($list as $key => $value): ?>
										<?php 
											$linkEdit = my_library::admin_site().'page/edit/'.$value['id'];
											...
											<td><h4><?= $value['page_title']?></h4>
												<div style="height: 20px;">
													<div class="actionhover">
														<a href="<?= $linkEdit.'?lang='.$flanguage['lang_code']?>" class="text-primary"><?= lang('edit')?></a> | <a href="<?= $linkView?>" class="text-primary" target="_blank"><?= lang('view')?></a> | <a href="javascript:void(0)" onclick="confirm_delete(<?= $value['id']?>,'<?= lang('page')?>')" class="text-danger"><?= lang('delete')?></a>
													</div>
												</div>
											</td>
