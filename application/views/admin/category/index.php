							...
							<tbody>
								<?php if (!empty($list)): ?>
									<?php foreach ($list as $key => $value): ?>
										<?php 
											$linkEdit = my_library::admin_site().'category/edit/'.$value['id'];
											$linkView = base_url().$value['category_alias'];
											...
											<td><h3><?= $value['category_name']?></h3>
												<div style="height: 20px;">
													<div class="actionhover">
														<a href="<?= $linkEdit.'?lang='.$flanguage['lang_code']?>" class="text-primary"><?= lang('edit')?></a> | <a href="<?= $linkView?>" class="text-primary" target="_blank"><?= lang('view')?></a><?= $value['id'] > 1 ? ' | <a href="javascript:void(0)" onclick="confirm_delete('.$value['id'].',\''.lang('category').'\')" class="text-danger">'.lang('delete').'</a>' : '';?>
													</div>
												</div>
											</td>
											<td><cite>/<?= $value['category_alias']?></cite></td>
											...
											<td class="text-center">
												<?php if (!empty($listLanguage)): ?>
													<?php foreach ($listLanguage as $key => $vallang): ?>
														<a href="<?= $linkEdit.'?lang='.$vallang['language_code']?>"><img src="<?= my_library::base_file().'language/flag_'.$vallang['language_code'].'.png'?>" style="max-width: 20px;height: auto;"></a>
													<?php endforeach ?>
												<?php endif ?>
											</td>
										...
										<?php if (isset($value['subcate'])): ?>
											<?php foreach ($value['subcate'] as $key => $val): ?>
												<?php 
													$linkEdit = my_library::admin_site().'category/edit/'.$val['id'];
													$linkView = base_url().$val['category_alias'];
												...
												<tr class="showacction">
													<td style="padding-left: 20px;"><h4 class="text-success"><i class="fa fa-arrow-circle-right fa-lg"></i> <?= $val['category_name']?></h4>
														<div style="height: 20px;">
															<div class="actionhover">
																<a href="<?= $linkEdit.'?lang='.$flanguage['lang_code']?>" class="text-primary"><?= lang('edit')?></a> | <a href="<?= $linkView?>" class="text-primary" target="_blank"><?= lang('view')?></a><?= $val['id'] > 1 ? ' | <a href="javascript:void(0)" onclick="confirm_delete('.$val['id'].',\''.lang('category').'\')" class="text-danger">'.lang('delete').'</a>' : '';?>
															</div>
														</div>
													</td>
													<td><cite>/<?= $val['category_alias']?></cite></td>
												...
												<?php if (isset($val['sub_subcate'])): ?>
													<?php foreach ($val['sub_subcate'] as $key => $v): ?>
														<?php 
															$linkEdit = my_library::admin_site().'category/edit/'.$v['id'];
															$linkView = base_url().$v['category_alias'];
														...
														<tr class="showacction">
															<td style="padding-left: 40px;"><h5 class="text-primary"><i class="fa fa-arrow-circle-right fa-lg"></i> <?= $v['category_name']?></h5>
																<div style="height: 20px;">
																	<div class="actionhover">
																		<a href="<?= $linkEdit.'?lang='.$flanguage['lang_code']?>" class="text-primary"><?= lang('edit')?></a> | <a href="<?= $linkView?>" class="text-primary" target="_blank"><?= lang('view')?></a><?= $v['id'] > 1 ? ' | <a href="javascript:void(0)" onclick="confirm_delete('.$v['id'].',\''.lang('category').'\')" class="text-danger">'.lang('delete').'</a>' : '';?>
																	</div>
																</div>
															</td>
															<td><cite>/<?= $v['category_alias']?></cite></td>
