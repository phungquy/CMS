							...
							<tbody>
								...
								<?php if (!empty($list)): ?>
									...
									<?php foreach ($list as $key => $value): ?>
										...
										$linkEdit = my_library::admin_site().'config/edit/'.$value['id'];
										...
										<tr class="showacction">
											...
											<td><?= $value['config_name']?>
												...
												<div style="height: 20px;">
													...
													<div class="actionhover">
														...
														<a href="javascript:void(0)" onclick="quickedit('<?= $value['id_lang']?>','<?= $value['config_name']?>')" class="text-primary"><?= lang('quickedit')?></a> | <a href="<?= $linkEdit.'?lang='.$flanguage['lang_code']?>" class="text-primary"><?= lang('edit')?></a> | <a href="javascript:;" data-id="<?= $value['id']?>" data-name="<?= lang('config')?>" class="text-danger delete"><?= lang('delete')?></a>
											...
											<td id="value<?= $value['id_lang']?>" style="word-break: break-word;"><p><?= $value['config_value']?></p></td>

											<td class="text-center"><code><?= $value['config_code']?></code></td>

											<td class="text-center"><span class="label label-<?= $status['color']?>"><?= $status['name']?></span>

											<?php if ($value['config_all'] == 1): ?>

												<br><span class="label label-primary"><?= lang('allpage')?></span>
