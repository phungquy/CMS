							...
							<tbody>
								<?php if (!empty($list)): ?>
									<?php foreach ($list as $key => $value): ?>
										...
										$linkEdit = my_library::admin_site().'album/edit/'.$value['id'];
											...
											<td><?= $value['album_name']?>
												<div style="height: 20px;">
													<div class="actionhover">
														<a href="<?= $linkEdit.'?lang='.$flanguage['lang_code']?>" class="text-primary"><?= lang('edit')?></a> | <a href="<?= $linkDetailPhoto?>" class="text-success"><?= lang('detailphoto')?></a> | <a href="javascript:;" onclick="confirm_delete(<?= $value['id']?>,'<?= lang('album')?>')" class="text-danger"><?= lang('delete')?></a>
													</div>
												</div>
											</td>
