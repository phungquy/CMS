						...
						<table class="table table-hover">
							<thead>
								<tr>
									...
									<th><?= lang('status')?></th>
									<th class="text-center"><?= lang('orderby')?></th>
							...
							<tbody>
								<?php if (!empty($list)): ?>
									<?php foreach ($list as $key => $value): ?>
										...
										$linkEdit = my_library::admin_site().'module/edit/'.$value['id'];
										...
										<tr class="showacction" <?= $opacity?>>
											...
											<td style="width: 400px;"><h3><?= $value['module_name']?></h3>
												<div style="height: 20px;">
													<div class="actionhover">
														<a href="<?= $linkEdit.'?lang='.$flanguage['lang_code']?>" class="text-primary"><?= lang('edit')?></a> | <a href="javascript:void(0)" onclick="confirm_delete(<?= $value['id']?>)" class="text-danger"><?= lang('delete')?></a>
													</div>
												</div>
											</td>
											<td>
											...
											<td class="text-center"><h3><?= $value['module_orderby']?></h3></td>
										...
										<?php if (isset($value['submodule'])): ?>
											<?php foreach ($value['submodule'] as $key => $val): ?>
												...
												$linkEdit = my_library::admin_site().'module/edit/'.$val['id'];
												...
												<tr class="showacction" <?= $opacitysub?>>
													...
													<td style="width: 400px;"><h4 style="margin-left: 30px;color: #16a085"><i class="fa fa-arrow-circle-right"></i> <?= $val['module_name']?></h4>
														<div style="height: 20px;margin-left: 30px;">
															<div class="actionhover">
																<a href="<?= $linkEdit.'?lang='.$flanguage['lang_code']?>" class="text-primary"><?= lang('edit')?></a> | <a href="javascript:void(0)" onclick="confirm_delete(<?= $val['id']?>)" class="text-danger"><?= lang('delete')?></a>
															</div>
														</div>
													</td>
													<td>
													...
													<td class="text-center"><h4><?= $val['module_orderby']?></h4></td>
													<td class="text-center">
														<?php if (!empty($listLanguagesub)): ?>
															<?php foreach ($listLanguagesub as $vallangsub): ?>
																<a href="<?= $linkEdit.'?lang='.$vallangsub['language_code']?>"><img src="<?= my_library::base_file().'language/flag_'.$vallangsub['language_code'].'.png'?>" style="max-width: 20px;height: auto;"></a>
															<?php endforeach ?>
														<?php endif ?>
													</td>
													<td></td>
